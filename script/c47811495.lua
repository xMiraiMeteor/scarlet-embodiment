...
function s.spfilter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0x322) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsCode(id)
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(s.spfilter,tp,LOCATION_GRAVE|LOCATION_REMOVED,0,1,e:GetHandler(),e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tg=Duel.SelectTarget(tp,s.spfilter,tp,LOCATION_GRAVE|LOCATION_REMOVED,0,1,1,e:GetHandler(),e,tp)
